...
export default function FeaturesSectionEditor({ data, onChange }: FeaturesSectionEditorProps) {
  const [currentSection, setCurrentSection] = useState<FeaturesSectionData>({
    section_title: '',
    subtitle: '',
    title: '',
    description: '',
  ...
  const handleFieldChange = (field: keyof FeaturesSectionData, value: string | Feature[] | File) => {
  ...
  const addNewSection = () => {
    const newSection: FeaturesSectionData = {
      section_title: '',
      subtitle: '',
      title: '',
      description: '',
  ...
  const deleteSection = async (index: number) => {
    ...
    if (sectionToDelete.id) {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/v1/features-sections/${sectionToDelete.id}`, {
          method: 'DELETE',
          headers: {
        ...
        if (response.ok) {
          ...
          alert('Features section deleted successfully!');
        } else {
          alert('Failed to delete features section');
        }
      } catch (error) {
        console.error('Error deleting features section:', error);
        alert('Error deleting features section');
      }
    } else {
        ...
        <>
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-900">
              Edit Features Section {editingIndex !== null ? editingIndex + 1 : ''}
            </h2>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500"
              placeholder="Business analytics subtitle"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500"
              placeholder="Accelerate your workflow and minimise your time"
            />
          </div>

...
import { useState, useEffect, useRef, useImperativeHandle, forwardRef, ReactElement, useCallback } from "react";
...
const CareersEditor = forwardRef<CareersEditorRef, {}>((props, ref): ReactElement => {
    ...
    useEffect(() => {
        console.log("=== Support Section Data Update ===");
        console.log("currentSupportSection:", currentSupportSection);
        console.log("currentSupportSection?.id:", currentSupportSection?.id);
        console.log("currentSupportSection?.section_title:", currentSupportSection?.section_title);
        console.log("currentSupportSection?.title:", currentSupportSection?.title);
        
        if (currentSupportSection) {
            // Always use database values, even if they're empty strings
            // Only pass string values to getImageUrl (File objects are for uploads, not display)
            const callIconValue = typeof currentSupportSection.call_icon === 'string' 
                ? currentSupportSection.call_icon 
                : null;
            const emailIconValue = typeof currentSupportSection.email_icon === 'string' 
                ? currentSupportSection.email_icon 
                : null;
            ...
            const updatedData = {
                ...
                callIcon: getImageUrl(callIconValue),
                ...
                emailIcon: getImageUrl(emailIconValue),
            ...
            console.log("Updating supportData with:", updatedData);
            setSupportData(updatedData);
        } else {
            // No support section data - clear form
            console.log("No currentSupportSection, clearing form");
            setSupportData({
                sectionTitle: "",
                title: "",
                description: "",
    ...
    }, [currentSupportSection, getImageUrl]);

    const fetchCareerCards = async () => {
        try {
            console.log("Fetching career cards from API...");
            const response = await fetch("http://localhost:5000/api/v1/career-cards", {
                method: "GET",
                headers: {
            ...
            console.log("Career cards API response status:", response.status);

            if (response.ok) {
                const data = await response.json();
                console.log("Career cards API response data:", data);
                ...
                console.warn("Career cards API returned non-OK status:", response.status);
                const errorText = await response.text().catch(() => "Unknown error");
                console.warn("Career cards API error response:", errorText);
            }
        } catch (err) {
            // Silently handle connection errors - port 5000 API may not be running
            if (err instanceof TypeError && err.message.includes('Failed to fetch')) {
                console.debug("Career cards API unavailable (port 5000) - this is expected if the service isn't running");
            } else {
                console.error("Error fetching career cards:", err);
            }
            // Don't throw - just log the error so it doesn't break the component
            // The career cards section will just show empty if the API is unavailable
        }
    };
    ...
    const handleUpdateCareerCard = async (cardId: number, cardData: any) => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/v1/career-cards/${cardId}`, {
                method: "POST",
                headers: {
                    "Accept": "application/json",
                    ...(token && { Authorization: `Bearer ${token}` }),
    ...
    const handleDeleteCardImage = async (cardId: number) => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/v1/career-cards/${cardId}/field/image`, {
                method: "DELETE",
                headers: {
                    "Accept": "application/json",
                    ...(token && { Authorization: `Bearer ${token}` }),
    ...
    const handleSaveAdd = async () => {
        try {
            ...
            formData.append("sort_order", (careerCards ? careerCards.length + 1 : 1).toString());
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/v1/career-cards/${editingCard}`, {
                method: "POST",
                headers: {
                    "Accept": "application/json",
                    ...(token && { Authorization: `Bearer ${token}` }),
    ...
    const handleAddNewFaq = async () => {
        ...
        console.log("Current faqs length:", faqs ? faqs.length : 0);
        ...
        try {
            const result = await handleCreateFAQSection({
                ...
                is_active: true,
                sort_order: (faqs ? faqs.length : 0) + 1,
            });
    ...
    const handleSaveSupportSection = async () => {
        ...
        try {
            ...
            const convertImageUrl = (imageUrl: string | null) => {
                ...
                if (imageUrl.startsWith('http://localhost:5000/storage/')) {
                    return imageUrl.replace('http://localhost:5000/storage/', '');
                }
            ...
            if (result) {
                ...
                // Refresh the support sections list
                await fetchSupportSections();
                // Always fetch the specific section that was just saved to ensure we have the latest data
                if (result.id) {
                    console.log("Fetching updated support section:", result.id);
                    await fetchSupportSection(result.id);
                }
            }
        } catch (err) {
    ...
    const fetchHeroSection = async () => {
        try {
            ...
            console.log("Hero section API response status:", response.status);

            if (response.ok) {
                const data = await response.json();
                console.log("Hero section API response data:", data);
                if (data.success && data.data?.career_page_hero_section) {
                    ...
                    const getHeroImageUrl = (imagePath: string | null | undefined): string | null => {
                        if (!imagePath) return null;
                        if (imagePath.startsWith("http")) return imagePath;
                        if (imagePath.startsWith("/storage") || imagePath.startsWith("/")) {
                            return `http://localhost:8000${imagePath}`;
                    ...
                    setGeneralData({
                        ...
                        image: null,
                        imageUrl: getHeroImageUrl(heroData.image),
                    });
                    console.log("Hero section data loaded successfully");
                } else {
                    console.warn("Hero section API response structure unexpected:", data);
                }
            } else {
                console.warn("Hero section API returned non-OK status:", response.status);
                const errorText = await response.text().catch(() => "Unknown error");
                console.warn("Hero section API error response:", errorText);
                // Don't set error state - just log it so the form can still be used
            }
        } catch (err) {
            // Silently handle connection errors - port 5000 API may not be running
            if (err instanceof TypeError && err.message.includes('Failed to fetch')) {
                console.debug("Hero section API unavailable (port 5000) - this is expected if the service isn't running");
            } else {
                console.error("Error fetching hero section:", err);
            }
            // Don't set error state - just log it so the form can still be used
            // The form will work with default/empty values if API is unavailable
        }
    };
    ...
    const handleSaveHeroSection = async () => {
        ...
        try {
            ...
            if (err instanceof TypeError && err.message.includes('Failed to fetch')) {
                const errorMsg = "Cannot connect to hero section API (port 8000). Please ensure the API server is running.";
                console.error("Error saving hero section:", errorMsg);
                setError(errorMsg);
            } else {
                ...
                <div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {careerCards && careerCards.length > 0 ? careerCards.map((card) => (
                                ...
                                <div className="col-span-full text-center py-8 text-gray-500">
                                    No career cards found. Add your first career card to get started.
                                </div>
                            )}
                        </div>
                    )}
                                            ...
                                            <label className="cursor-pointer">
                                                ...
                                                <div className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-gray-200">
                                                    {job.image ? (
                                                        ...
                                                        <img
                                                            src={job.imageUrl.startsWith('http') ? job.imageUrl : `http://localhost:5000${job.imageUrl}`}
                                                            alt={job.title}
                                                            className="w-24 h-24 object-cover"
                                                        />
                                                    ) : (
                ...
                <div className="space-y-8">
                    ...
                    {supportLoading && (
                        <div className="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded-lg">
                            Loading support section data...
                        </div>
                    )}

                    {/* No Data Message */}
                    {!supportLoading && !currentSupportSection && (
                        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg">
                            <div className="flex items-center justify-between">
                                <span>No support section data found. The form below will allow you to create a new support section.</span>
                                <button
                                    onClick={() => {
                    ...
                    {/* Support Section Selector - Only show if multiple sections exist */}
                    {supportSections.length > 1 && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                Select Support Section to Edit:
                            </label>
                            <select
                                value={currentSupportSection?.id || ""}
                                onChange={(e) => {
                                    const sectionId = parseInt(e.target.value);
                                    const section = supportSections.find(s => s.id === sectionId);
                                    if (section) {
                                        fetchSupportSection(sectionId);
                                    }
                                }}
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-gray-900 bg-white"
                            >
                                {supportSections.map((section) => (
                                    <option key={section.id} value={section.id}>
                                        {section.section_title || section.title || `Support Section #${section.id}`}
                                    </option>
                                ))}
                            </select>
                            <div className="mt-2 text-xs text-gray-600">
                                Currently editing: <strong>ID {currentSupportSection?.id}</strong> - {currentSupportSection?.section_title || currentSupportSection?.title || "No title"}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-gray-100 rounded-lg p-6">
                        <h3 className="text-xl font-bold text-gray-900 mb-6">
                            Support Section {currentSupportSection?.id ? `(ID: ${currentSupportSection.id})` : ""}
                        </h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    key={`section-title-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    key={`title-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <textarea
                                    key={`description-${currentSupportSection?.id || 'new'}`}
                    ...
                    <div className="bg-gray-100 rounded-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    key={`call-title-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <textarea
                                    key={`call-description-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <input
                                    type="tel"
                                    key={`call-phone-${currentSupportSection?.id || 'new'}`}
                    ...
                    <div className="bg-gray-100 rounded-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    key={`email-title-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <textarea
                                    key={`email-description-${currentSupportSection?.id || 'new'}`}
                            ...
                            <div>
                                ...
                                <input
                                    type="email"
                                    key={`email-address-${currentSupportSection?.id || 'new'}`}
                    ...
                    <div className="bg-gray-100 rounded-lg p-6">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex gap-4 text-sm">
                                <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full">
                                    Total: {statistics ? statistics.total_submissions : 0}
                                </div>
                                <div className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full">
                                    Unread: {statistics ? statistics.unread_submissions : 0}
                                </div>
                                <div className="bg-green-100 text-green-800 px-3 py-1 rounded-full">
                                    Read: {statistics ? statistics.read_submissions : 0}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mb-4 flex gap-4">
                            <input
                                type="text"
                                placeholder="Search by name, email, phone, job location, or message..."
                                onChange={(e) => {
                                    const searchTerm = e.target.value;
                                    if (searchTerm.length > 2 || searchTerm.length === 0) {
                                        fetchContactSubmissions(1, undefined, searchTerm || undefined);
                                    }
                                }}
                            ...
                            <select
                                onChange={(e) => {
                                    ...
                                    fetchContactSubmissions(1, status);
                                }}
                        ...
                        {(contactSuccess || contactError) && (
                            <div className={`mb-4 p-4 rounded-lg border ${contactSuccess ? 'bg-green-100 border-green-400 text-green-800' : 'bg-red-100 border-red-400 text-red-800'}`}>
                                {contactSuccess || contactError}
                            </div>
                        )}
                            ...
                            <table className="min-w-full bg-white border border-gray-200 rounded-lg divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">NAME</th>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">PHONE</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">JOB LOCATION</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">PREFERRED CONTACT</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">BEST TIME</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">MESSAGE</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">SUBMITTED</th>
                                    ...
                                    {contactLoading ? (
                                        ...
                                        contactSubmissions.map((submission) => (
                                            <tr key={submission.id} className={`hover:bg-gray-50 transition-colors ${!submission.is_read ? 'bg-blue-50' : 'bg-white'}`}>
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 border-b border-gray-200">
                                                    {submission.job_location || <span className="text-gray-400">-</span>}
                                                </td>
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 border-b border-gray-200">
                                                    {submission.preferred_contact_method ? (
                                                        <span className="capitalize">{submission.preferred_contact_method}</span>
                                                    ) : (
                                                        <span className="text-gray-400">-</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 border-b border-gray-200">
                                                    {submission.best_time_to_contact ? (
                                                        <span className="capitalize">{submission.best_time_to_contact}</span>
                                                    ) : (
                                                        <span className="text-gray-400">-</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-4 text-sm text-gray-900 max-w-xs border-b border-gray-200">
                                                    <div className="truncate" title={submission.message}>
                                                        {submission.message || <span className="text-gray-400">-</span>}
                                                    </div>
                                                </td>
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900 border-b border-gray-200">
                        ...
                        {pagination && pagination.total > 0 && (
                            <div className="mt-4 flex justify-between items-center text-sm text-gray-600">
                                <div>
                                    Showing {pagination ? pagination.from : 0} to {pagination ? pagination.to : 0} of {pagination ? pagination.total : 0} submissions
                                </div>
                                <div className="flex gap-2">
                                    {pagination && pagination.current_page > 1 && (
                                        <button
                                            onClick={() => fetchContactSubmissions(pagination ? pagination.current_page - 1 : 1)}
